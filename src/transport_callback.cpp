...
/**
 * The transport simple works by breaking up the data into segments and sending them immediately. The initial packet is small and only contains the new data's number of
 * segments, bytes, checksum and ID. The following packets/segments contain the data.
 * The receiving end will store all data in a buffer until all segments are received. Once all segments are received, the data is checked for corruption and then forwarded.
 *
 * Specifically each packet MUST append the following 8 bytes: [srcPortHigh, srcPortLow, dstPortHigh, dstPortLow, orderHigh, orderLow, ID, transportIdentifier]
 * A packet with order 0 is the info packet and contains the number of segments, bytes, checksum and ID.
 */
...
namespace VCTR
{
    ...
    namespace Net
    {
        ...
        {
            receiveTopicSubr_.setCallback(this, &TransportCallback::receivePacketCallback);
            transmitTopicSubr_.setCallback(this, &TransportCallback::send);
        }

        TransportCallback::TransportCallback(uint16_t port, Network_Interface &node) : TransportCallback(port)
        {
            setNetworkNode(node);
        }
        ...
        {
            receiveTopicSubr_.subscribe(node.getReceiveTopic());
            netNodeTransmitTopic_.subscribe(node.getTransmitTopic());
        }
         ...
         */
        void TransportCallback::send(const TransportData &data)
        {
            ...
            uint16_t numBytes = data.data.size();
            ...
            sendSegment(packet, 0, data.dstAddress, data.dstPort, sendingID_);
            ...
            for (uint16_t i = 0; i < numSegments; i++)
            {
                ...
                for (uint16_t j = 0; j < segmentSize; j++)
                {
                    ...
                    packet.payload.placeBack(data.data[i * segmentSize + j]);
                }

                VRBS_MSG("Sending data segment %d. \n", i + 1);

                sendSegment(packet, i + 1, data.dstAddress, data.dstPort, sendingID_);
            }
        ...
        void TransportCallback::sendSegment(NetworkPacket &segment, uint16_t order, uint16_t dstAddress, uint16_t dstPort, uint8_t id)
        {
            ...
            netNodeTransmitTopic_.publish(segment);
        }
        ...
        void TransportCallback::receivePacketCallback(const NetworkPacket &packet)
        {
            ...
            if (id != rcvID_)
            { // We are receiving new data. Discard old data and restart.
                ...
                if (numSegments == 0 || numBytes == 0 || numBytes < numSegments)
                { // No data to receive or something is wrong with the data. Discard.
            ...
            if (curSegment_ == numSegments_)
            {
                ...
                // Reconstruct the data by placing each segments data from segmentBuffer_ in the correct order inside the receivedData_ buffer.
                ...
                uint8_t crc = Core::computeCrc(data.data, 0);
                if (crc != checksum_ || numBytes_ != data.data.size())
                { // Data is corrupt. Discard.

                    LOG_MSG("Received data is corrupt. CRC rcv: %d, Expected: %d. Data length: %d. \n", crc, checksum_, data.data.size());
                    receivedData_.clear();
                }
                else
                { // publish the data

                    VRBS_MSG("Received data is correct. CRC rcv: %d, Expected: %d. Data length: %d. \n", crc, checksum_, data.data.size());
