...
void TopicIO::disconnect() { receiveSubr_.unsubscribe(); }

// ############# Below are the input functions ################

HAL::IO_TYPE_t TopicIO::getInputType() const { return HAL::IO_TYPE_t::TOPIC; }

bool TopicIO::setInputParam(HAL::IO_PARAM_t param, int32_t value) {
  LOG_MSG("TopicIO setInputParam: Something attempted to change param values. "
          "This is not supported in topicIO! Param: %d, Value: %d\n",
          param, value);
  return false;
}
...
size_t TopicIO::readable() {
  ...
  // receiveBuffer_.getFront(), receiveBuffer_.getBack(), receiveBuffer_.size(),
  // this);
...
HAL::IO_TYPE_t TopicIO::getOutputType() const { return HAL::IO_TYPE_t::TOPIC; }

bool TopicIO::setOutputParam(HAL::IO_PARAM_t param, int32_t value) {
  LOG_MSG("TopicIO setOutputParam: Something attempted to change param values. "
          "This is not supported in topicIO! Param: %d, Value: %d\n",
          param, value);
  return false;
}

size_t TopicIO::writable() { return 255; }
