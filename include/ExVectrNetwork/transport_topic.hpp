...
namespace VCTR
{
    ...
    namespace Net
    {
        ...
        /**
         * @brief   The topic transport system can be used to link topics together over a network. Enabling easy communication between different systems.
         * @note    This does not guarantee delivery of messages. Also be aware of the datarate of the datalink/physical layer and buffering limits.
         * @tparam  TYPE The type of data to be sent over the network.
         */
        template <typename TYPE>
        class TransportTopic {
        private:
            ...
            Core::Callback_Subscriber<TransportCallback::TransportData, TransportTopic<TYPE>> transportSubr_;
        ...
        public:
            ...
            /**
             * @param channel The port to use for this topic for bidirectional communication. Only a single topic can be used per channel. (Theoretically multiple topics can be used on the same port if the data length is different.)
             ...
             * @param noReceive If true then this topic will not receive data (Send only).
             */
            TransportTopic(uint16_t channel, uint16_t dstAddress, Network_Interface &node, bool noReceive = false) :
             ...
             * @param noReceive If true then this topic will not receive data (Send only).
             */
            TransportTopic(uint16_t channel, uint16_t dstAddress, Network_Interface &node, Core::Topic<TYPE> &topic, bool noReceive = false) :
             ...
             */
            void setDstAddress(uint16_t dstAddress) {
                dstAddress_ = dstAddress;
            }
             ...
             * @param noReceive If true then this topic will not receive data (Send only).
             */
            void setNoReceive(bool noReceive = true) {
                noReceive_ = noReceive;
            }
             ...
             */
            void setTopic(Core::Topic<TYPE> &topic) {
                topicSubr_.subscribe(topic);
            }
        ...
        private:
            ...
            void receiveData(const Net::TransportCallback::TransportData &data) {
                ...
                if (noReceive_ || data.srcPort != transport_.getPort() || data.data.size() != sizeof(TYPE)) return;

                TYPE item;
                for (size_t i = 0; i < sizeof(TYPE); i++) ((uint8_t *)&item)[i] = data.data[i];
                topicSubr_.publish(item);

            }

            /**
             * @brief   Sends the given data to the given address and port using the transport layer.
             * @param data The data to send and the destination address and port. The source address and port will be added automatically.
             */
            void sendData(const TYPE &data) {
                ...
                for (size_t i = 0; i < sizeof(TYPE); i++) dataToSend.data.append(((uint8_t *)&data)[i]);
