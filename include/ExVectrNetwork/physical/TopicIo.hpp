...
private:
  ...
  Core::Callback_Subscriber<const Core::List<uint8_t> &, TopicIO> receiveSubr_;
...
public:
   ...
   */
  TopicIO(Core::Topic<const Core::List<uint8_t> &> &topic);

  /**
   * @brief Uses given topicIO for data transfer.
   * @param topicIO
   */
  void setTopicIO(Core::Topic<const Core::List<uint8_t> &> &topic);
  ...
  /**
   * @brief Disconnects connected topicIO
   */
  void disconnect();
   ...
   * @param endTransfer Set to false if doing mulitple reads. Last read should
   * have endTransfer set to true.
   *
   ...
   * @param endTransfer Set to false if doing mulitple writes. Last write should
   * have endTransfer set to true.
   *
   * @return number of bytes actually written.
   */
  size_t writeData(const void *data, size_t size,
                   bool endTransfer = true) override;

private:
  void receiveItem(const Core::List<uint8_t> &item);
};
