...
namespace VCTR
{
    ...
    namespace Net
    {
        ...
        /**
         * @brief   The simple and fast transport class. Similar to UDP it will break up large data and send it in segments, rebuilding them on the other end and checking for corruption. But does not guarantee delivery.
         *          This is mainly used as a basis for other transport methods, buyt can be used as is.\
         * @note    Due to the underlying systems also being based on callbacks, attempt to reduce the further number of callbacks to offload the datalink layer (Head of the chain of callbacks).
         */
        ...
        public:
            ...
            struct TransportData
            {
                ...
                uint16_t dstAddress;
                uint16_t dstPort;
                /// @brief The data being sent or received. The creator of this structure is responsible for managing the memory.
                Core::ListArray<uint8_t> data;
            };

        private:

            /// @brief A version control for the transport simple. This is added together with the transport identifier to not conflict with other transport protocols.
            static constexpr uint8_t transportSimpleVersion = 1;
            ...
            /// @brief The ID of the current data being sent. Must always be different than last.
            ...
            /// @brief A buffer to store the data being received until all segments are received.
            ...
            Core::Callback_Subscriber<TransportData, TransportCallback> transmitTopicSubr_;

            /// @brief Callback for receiving data from the network node.
            Core::Callback_Subscriber<NetworkPacket, TransportCallback> receiveTopicSubr_;
            /// @brief Publisher to send data to the network node.
            Core::Topic_Publisher<NetworkPacket> netNodeTransmitTopic_;
        ...
        public:
             ...
             */
            void send(const TransportData& data);

            /**
             * @brief   Gets the topic to receive data from the network node. Subscribe to this topic to receive data from the other end.
            ...
            /**
             * @brief   Gets the topic to transmit data to the network node. Publish to this topic to send data to the other end.
        ...
        private:
            ...
            /**
             * @brief   Sends a segment of data to the given address and port. Basically appends the required information to the data and sends it.
             * @param data The data to send.
             * @param dstAddress The destination address.
             * @param dstPort The destination port.
             * @param segmentID The ID of the segment being sent.
             */
            void sendSegment(NetworkPacket& segment, uint16_t order, uint16_t dstAddress, uint16_t dstPort, uint8_t id);
